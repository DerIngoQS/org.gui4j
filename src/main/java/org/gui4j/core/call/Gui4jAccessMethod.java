  ...
  Gui4jAccessMethod(ParseCtx parseCtx, Gui4jAccessImpl thisAccess, String accessPath) {
    ...
    if (lParenIndex != -1 && lParenIndex < endIndex) {
      ...
      while (true) {
        ...
        // Parameter expected
        Gui4jAccessImpl gui4jAccess =
            parseCtx.getGui4jCallParser().parseCallSequence(parseCtx, accessPath);
        /*
        getInstance(
            parseCtx,
    ...
    if (mField == null) {
      ...
      mMethod =
          parseCtx
              ...
              .getMethod(context, thisAccess.getResultClass(), methodName, arguments);
    } else {
      mMethod = null;
    }
  }
