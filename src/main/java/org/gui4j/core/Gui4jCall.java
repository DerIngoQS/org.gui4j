...
/**
 * Represents the interface for all reflection call. Method calls, field accesses, integer
 * constants, and also string constants are instances of this interface. Parsing of reflection calls
 * is done by
 */
public interface Gui4jCall extends Gui4jGetValue {
  ...
  /**
   * Method hasTriggerEvents. Returns <code>true</code> if the list of dependant events is
   * non-empty.
   *
   * @return boolean
   */
   ...
   *
   * @return a set containing the names of all actually used parameters in this call. The returned
   *     set is empty (not <code>null</code>) if no parameters were used.
   */
  Set getUsedParams();
  ...
  /**
   * Method getValueUseDefaultParam. Evaluates the reflection call with given value for the default
   * parameter (name "").
   *
   * @param baseInstance
   ...
   * @return Object
   */
  Object getValueUseDefaultParam(
      Gui4jCallBase baseInstance, Object defaultParamValue, Object defaultValue);

  /**
   * Method getConfigurationSource.
   *
   * @return String. The name of the XML-file where the reflection call was defined. This is usually
