...
/**
 * Supports methods to method declarations by most specific argument types There is always one
 * instance of this manager in order to cache method declarations.
 *
...
public final class Gui4jReflectionManager implements ErrorTags, Serializable {
        ...
        if (lastMethod == null) {
          ...
          if (moreSpecific(methods[i], lastMethod)) {
            ...
            throw new Gui4jUncheckedException.ProgrammingError(
                PROGRAMMING_ERROR_method_ambiguous, args);
          }
        }
      }
    }
  ...
  /**
   * Searches for the given class the most specific method. If no method is found, or if the result
   * is ambiguous, an exception is raised
   *
   * @param context
   * @param c
   * @param methodName
