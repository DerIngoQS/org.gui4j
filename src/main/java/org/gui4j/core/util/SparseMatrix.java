...
/**
 * Generic Class for sparsely populated matrices. The size of the matrix is dynamic, i.e. you can
 * specify arbitrary row and column parameters.
 *
 * <p>Another way to look at this class is as a "two-dimensional" map. Instead of a single key it
 * uses a row and a column element to store and retrieve data elements.
 *
 * <p>The public Interface Traverser allows to traverse the matrix generically. The Traverser object
 * is passed into <code>traverse(Traverser)</code> and is given the opportunity to work with each
...
public final class SparseMatrix implements Serializable {
  ...
  public static interface Traverser {
    /**
     * Called once for each element that is visited during traversal. There is no defined traversal
     * sequence. If the Traverser manipulates any one of row, col or value the effects on the matrix
     * are undefined.
     *
  ...
  /**
   * Returns the value found at the specified row and column of the matrix. Returns null if there's
   * no value for the specified row and column.
   *
   * @param row Object
   * @param col Object
   * @return Object
  ...
  /**
   * Traverses the matrix and calls <code>work()</code> on the supplied Traverser object for each
   * matrix element. The traversal sequence is undefined.
   *
