...
public final class Gui4jTypeCheck implements ErrorTags {
  ...
  public static void ensureType(
      Class resultClass, Class expectedType, String configurationName, String accessPath) {
    boolean valid = false;
    if (resultClass != null) {
    ...
    if (!valid) {
      if (accessPath != null) {
        ...
        throw new Gui4jUncheckedException.ResourceError(
            configurationName, -1, RESOURCE_ERROR_access_type_not_compatible, args);
      } else {
        Object[] args = {resultClass, expectedType};
        throw new Gui4jUncheckedException.ResourceError(
            configurationName, -1, RESOURCE_ERROR_type_not_compatible, args);
      }
    }
  }
}
