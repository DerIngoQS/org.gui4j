...
public class Gui4jListenerEdit extends Gui4jActionListener implements FocusListener {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (mSetValue != null) {
      ...
      textField.selectAll();
      log.debug("Asking ThreadManager to perform work for setValue."); // XXX
      mGui4jThreadManager.performWorkHighPriority(
          mGui4jController, work, paramMap, mGui4jComponentInstance);
    }
    ...
    if (mActionPerformed != null) {
      log.debug("Asking ThreadManager to perform work for actionCommand."); // XXX
  ...
  /**
   * This method can be used directly by client code to manually circumvent the problem where a
   * worker thread needs to work with current model values but the current value for an edit field
   * has not been set because no focus lost event has been triggered. In this case, the controller
   * can get the currently focused Swing component from {@link
   * java.awt.KeyboardFocusManager#getFocusOwner()}, check for the presence of a listener of this
   * type and call this method. <br>
   * Note: This method is regarded as a workaround for a problem that should really be solved
   * conceptually in Gui4j.
   */
  ...
  private void saveValue(boolean useWorkerThread) {
    if (mSetValue != null) {
      ...
      if (!fieldValue.equals(lastContent)) {
        ...
        textField.selectAll();
        mGui4jThreadManager.performWorkHighPriority(
            mGui4jController, work, paramMap, mGui4jComponentInstance, !useWorkerThread);
      }
    }
  }
}
