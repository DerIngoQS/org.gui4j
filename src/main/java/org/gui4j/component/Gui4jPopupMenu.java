...
public class Gui4jPopupMenu extends Gui4jJComponent {
    ...
    for (Iterator it = mMenuItems.iterator(); it.hasNext(); ) {
      ...
      if (gui4jSubComponent == null) {
        ...
        Gui4jComponentInstance subInstance =
            gui4jComponentInstance.getGui4jComponentInstance(gui4jSubComponent);
  ...
  /**
   * Shows the popup menu contained in <code>popupInstance</code>. Any unnessary separators are made
   * invisible prior to showing the popup menu (separators at the top or bottom of the menu as well
   * as duplicate separators).
   *
  ...
  public void show(Gui4jComponentInstance popupInstance, Component invoker, int x, int y) {
    ...
    for (int i = 0; i < elements.length; i++) {
      ...
      if (element instanceof JSeparator) {
        element.setVisible(
            !(previousVisibleElement == null || previousVisibleElement instanceof JSeparator));
      } else {
