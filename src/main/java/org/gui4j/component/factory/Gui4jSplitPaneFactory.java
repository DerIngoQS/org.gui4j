...
public final class Gui4jSplitPaneFactory extends Gui4jJComponentFactory {
  ...
  protected Gui4jJComponent defineGui4jJComponentBy(
    ...
    Gui4jQualifiedComponent gui4jComponentInPath1 =
        gui4jComponentContainer.extractGui4jComponent(element1);
    Gui4jQualifiedComponent gui4jComponentInPath2 =
        gui4jComponentContainer.extractGui4jComponent(element2);

    String orientationStr = e.attributeValue(ORIENTATION);
    int orientation =
        (orientationStr == null || orientationStr.equalsIgnoreCase(ORIENTATION_HORIZONTAL))
            ? JSplitPane.HORIZONTAL_SPLIT
            : JSplitPane.VERTICAL_SPLIT;
    boolean showDivider = gui4jComponentContainer.getBooleanAttrValue(e, SHOW_DIVIDER, true);

    Gui4jSplitPane gui4jSplitPane =
        new Gui4jSplitPane(
            gui4jComponentContainer,
            id,
            orientation,
            showDivider,
            gui4jComponentInPath1,
            gui4jComponentInPath2);

    gui4jSplitPane.definePropertySetter(
        LOCATION, getGui4jAccessInstance(Double.TYPE, gui4jSplitPane, e, LOCATION));
    gui4jSplitPane.definePropertySetter(
        RESIZE_WEIGHT, getGui4jAccessInstance(Double.TYPE, gui4jSplitPane, e, RESIZE_WEIGHT));

    gui4jSplitPane.definePropertySetter(
        DIVIDERSIZE, getGui4jAccessInstance(Integer.TYPE, gui4jSplitPane, e, DIVIDERSIZE));
  ...
  public void addToplevelAttributes(List attrList, Filter filter) {
    ...
    if (filter == null || filter.takeIt(Gui4jSplitPaneFactory.class)) {
      attrList.add(
          new Attribute(
              ORIENTATION,
              new AttributeTypeEnumeration(s, ORIENTATION_HORIZONTAL),
              IMPLIED,
              false));
      ...
      attrList.add(
          new Attribute(
              SHOW_DIVIDER, AttributeTypeEnumeration.getBooleanInstance(true), IMPLIED, false));
    }
  }
