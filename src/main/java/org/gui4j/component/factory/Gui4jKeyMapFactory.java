...
/**
 * Factory fuer KeyMap. Eine KeyMap enthaelt eine Menge von input und action Elementen. Ein input
 * Element definiert ein Mapping von einem Keystroke auf eine action. Und ein action Element
 * definiert das Mapping von einer Action auf einen ActionCommand. Der Einfachheit halber werden
 * Key-Bindings wie Swing-Instanzen verwendet.
 */
public class Gui4jKeyMapFactory extends Gui4jComponentFactory {
   ...
   */
  public Gui4jComponent defineBy(
    ...
    for (Iterator it = e.elements().iterator(); it.hasNext(); ) {
      ...
      if (child.getName().equals(INPUT)) {
        int focusCondition =
            getMapValue(
                gui4jComponentContainer, child, AFOCUSCONDITION, mFocusCondition, COND_FOCUSED);
  ...
  public void addInnerAttributes(String elementName, List list) {
    if (INPUT.equals(elementName)) {
      Attribute[] attrs = {
        new Attribute(
            AFOCUSCONDITION, new AttributeTypeEnumeration(mFocusCondition), REQUIRED, false),
    ...
    if (ADDKEYMAP.equals(elementName)) {
      Attribute[] attrs = {new Attribute(AID, new AttributeTypeID(), REQUIRED, false)};
      list.addAll(Arrays.asList(attrs));
    }
  }
